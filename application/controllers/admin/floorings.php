...
class Floorings extends MY_Controller
{
	...
	{	
		...
		$this->template->title(lang('web_list_of', array(':name' => 'floorings')));
		...
		$this->template->set('floorings', Flooring::paginate_all($config['per_page'], $page));
		...
		//Load view
		$this->template->build('floorings/list');
	}
	...
	function create($page = NULL) 
	{
		...
		$form_data_aux 	= 	array();
		$this->template->title(lang('web_create_t', array(':name' => 'floorings')));
		...
		{
			//Load view
			$this->template->build('floorings/create');	
		}
		else
		{
			//Validation OK!
			...
			$flooring = Flooring::create($form_data);

			if ( $flooring->is_valid() ) // the information has therefore been successfully saved in the db
			{
				$this->session->set_flashdata('message', array( 'type' => 'success', 'text' => lang('web_create_success') ));
			}
			
			if ( $flooring->is_invalid() )
			{
				$this->session->set_flashdata('message', array( 'type' => 'error', 'text' => $flooring->errors->full_messages() ));
			}

			redirect('/admin/floorings/');
	...
	function edit($id = FALSE, $page = 1) 
	{
		...
		//create control variables
		$this->template->title(lang('web_edit_t', array(':name' => 'floorings')));
		...
		if ($this->form_validation->run() == FALSE) // validation hasn't been passed
		{
			...
			$this->template->set('flooring', Flooring::find_by_id($id));
		...
		{	
			...
			$flooring = Flooring::find($this->input->post('id', TRUE));
			$flooring->update_attributes($form_data);
	...
	private function set_paginate_options()
	{
		...
		$config['base_url'] = site_url() . 'admin/floorings';

		$config['use_page_numbers'] = TRUE;

	    $config['per_page'] = 10;

		$config['total_rows'] = Flooring::count();
