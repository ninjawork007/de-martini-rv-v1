...
class Furnitures extends MY_Controller
{
	...
	{	
		...
		$this->template->title(lang('web_list_of', array(':name' => 'furnitures')));
		...
		$this->template->set('furnitures', Furniture::paginate_all($config['per_page'], $page));
		...
		//Load view
		$this->template->build('furnitures/list');
	}
	...
	function create($page = NULL) 
	{
		...
		$form_data_aux 	= 	array();
		$this->template->title(lang('web_create_t', array(':name' => 'furnitures')));
		...
		{
			//Load view
			$this->template->build('furnitures/create');	
		}
		else
		{
			//Validation OK!
			...
			$furniture = Furniture::create($form_data);

			if ( $furniture->is_valid() ) // the information has therefore been successfully saved in the db
			{
				$this->session->set_flashdata('message', array( 'type' => 'success', 'text' => lang('web_create_success') ));
			}
			
			if ( $furniture->is_invalid() )
			{
				$this->session->set_flashdata('message', array( 'type' => 'error', 'text' => $furniture->errors->full_messages() ));
			}

			redirect('/admin/furnitures/');
	...
	function edit($id = FALSE, $page = 1) 
	{
		...
		//create control variables
		$this->template->title(lang('web_edit_t', array(':name' => 'furnitures')));
		...
		if ($this->form_validation->run() == FALSE) // validation hasn't been passed
		{
			...
			$this->template->set('furniture', Furniture::find_by_id($id));
		...
		{	
			...
			$furniture = Furniture::find($this->input->post('id', TRUE));
			$furniture->update_attributes($form_data);
			...
			if ( $furniture->is_valid()) // the information has therefore been successfully saved in the db
			{
				...
				redirect("/admin/furnitures/".$page);
			}
	...
	private function set_paginate_options()
	{
		...
		$config['base_url'] = site_url() . 'admin/furnitures';

		$config['use_page_numbers'] = TRUE;

	    $config['per_page'] = 10;

		$config['total_rows'] = Furniture::count();
