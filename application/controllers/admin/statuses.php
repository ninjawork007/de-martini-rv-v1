...
class Statuses extends MY_Controller
{
	...
	{	
		...
		$this->template->title(lang('web_list_of', array(':name' => 'statuses')));
		...
		$this->template->set('statuses', Status::paginate_all($config['per_page'], $page));
		...
		//Load view
		$this->template->build('statuses/list');
	}
	...
	function create($page = NULL) 
	{
		...
		$form_data_aux 	= 	array();
		$this->template->title(lang('web_create_t', array(':name' => 'statuses')));
		...
		{
			//Load view
			$this->template->build('statuses/create');	
		}
		else
		{
			//Validation OK!
			...
			$status = Status::create($form_data);

			if ( $status->is_valid() ) // the information has therefore been successfully saved in the db
			{
			...
			{
				$this->session->set_flashdata('message', array( 'type' => 'error', 'text' => $status->errors->full_messages() ));
			}
	...
	function edit($id = FALSE, $page = 1) 
	{
		...
		//create control variables
		$this->template->title(lang('web_edit_t', array(':name' => 'statuses')));
		...
		if ($this->form_validation->run() == FALSE) // validation hasn't been passed
		{
			...
			$this->template->set('status', Status::find_by_id($id));
		...
		{	
			...
			$status = Status::find($this->input->post('id', TRUE));
			$status->update_attributes($form_data);
	...
	private function set_paginate_options()
	{
		...
		$config['base_url'] = site_url() . 'admin/statuses';

		$config['use_page_numbers'] = TRUE;

	    $config['per_page'] = 10;

		$config['total_rows'] = Status::count();
