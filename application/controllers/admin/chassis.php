...
class Chassis extends MY_Controller
{
	...
	{	
		...
		$this->template->title(lang('web_list_of', array(':name' => 'chassis')));
		...
		$this->template->set('chassis', Chassis::paginate_all($config['per_page'], $page));
		...
		//Load view
		$this->template->build('chassis/list');
	}
	...
	function create($page = NULL) 
	{
		...
		$form_data_aux 	= 	array();
		$this->template->title(lang('web_create_t', array(':name' => 'chassis')));
		...
		{
			//Load view
			$this->template->build('chassis/create');	
		}
		else
		{
			//Validation OK!
			...
			$chassis = Chassis::create($form_data);

			if ( $chassis->is_valid() ) // the information has therefore been successfully saved in the db
			{
				$this->session->set_flashdata('message', array( 'type' => 'success', 'text' => lang('web_create_success') ));
			}
			
			if ( $chassis->is_invalid() )
			{
				$this->session->set_flashdata('message', array( 'type' => 'error', 'text' => $chassis->errors->full_messages() ));
			}

			redirect('/admin/chassis/');
	...
	function edit($id = FALSE, $page = 1) 
	{
		...
		//create control variables
		$this->template->title(lang('web_edit_t', array(':name' => 'chassis')));
		...
		if ($this->form_validation->run() == FALSE) // validation hasn't been passed
		{
			...
			$this->template->set('chassis', Chassis::find_by_id($id));
		...
		{	
			...
			$chassis = Chassis::find($this->input->post('id', TRUE));
			$chassis->update_attributes($form_data);
	...
	private function set_paginate_options()
	{
		...
		$config['base_url'] = site_url() . 'admin/chassis';

		$config['use_page_numbers'] = TRUE;

	    $config['per_page'] = 10;

		$config['total_rows'] = Chassis::count();
