...
<p>
      <label class='labelform' for="group_id">Group<span class="required">*</span></label>
	<select name='group_id' id='group_id' size='1'>
		<option value=''><?=lang('web_choose_option')?></option>
		<?php foreach ($array_groups as $item): ?>
			<option value="<?=$item->id;?>" <?php if(does_id_exist($item->id, ($updType == 'edit') ? $user->groups : array())): ?>selected="selected"<?php endif;?>><?=$item->name;?></option>
		<?php endforeach ?>
	</select>
